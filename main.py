    ...
    def __init__(self):
        ...
        self.data = {
            'ru': {
                ...
                'Найденные "ошибки":': 'Найденные "ошибки":',
                ...
                'Табличное отображение ошибок': 'Табличное отображение ошибок',
            ...
            'en': {
                ...
                'Количество символов (без пробелов):': 'Characters (no spaces):',
                'Найденные "ошибки":': 'Found "errors":',
                ...
                'Табличное отображение ошибок': 'Error table display',
...
class ResultTab(QWidget):
    def __init__(self, tr):
        ...
        self.table.setHorizontalHeaderLabels([self.tr('Строка'), self.tr('Позиция'), self.tr('Сообщение')])
    ...
    def add_result(self, row, position, message):
        row_count = self.table.rowCount()
        self.table.insertRow(row_count)
        self.table.setItem(row_count, 0, QTableWidgetItem(str(row)))
        self.table.setItem(row_count, 1, QTableWidgetItem(str(position)))
        self.table.setItem(row_count, 2, QTableWidgetItem(message))
...
class TextEditor(QMainWindow):
    ...
    def initUI(self):
        ...
        self.error_table_tab = ResultTab(self.tr)
        
        self.result_tabs.addTab(self.text_result_tab, self.tr("Текстовый ввод"))
        self.result_tabs.addTab(self.error_table_tab, self.tr("Таблица ошибок"))
    ...
    def retranslateUi(self):
        ...
        self.result_tabs.setTabText(1, self.tr("Таблица ошибок"))
    ...
    def show_text_info(self, text_type):
        info_texts = {
            'Постановка задачи': 'Разработать текстовый редактор с языковым процессором, выполняющим анализ введенного текста.',
            'Грамматика': 'Используется контекстно-свободная грамматика для анализа структуры текста.',
            'Классификация грамматики': 'Грамматика относится к классу LL(1).',
            'Метод анализа': 'Используется метод рекурсивного спуска.',
            'Тестовый пример': 'Пример текста: "Hello world! This is a test."',
            'Список литературы': '1. Ахо А., Сети Р., Ульман Дж. Компиляторы: принципы, технологии и инструменты.',
            'Исходный код программы': 'Исходный код доступен в файле main.py'
        }
    ...
    def start_analyzer(self):
        ...
        for i, line in enumerate(lines, 1):
            if '!' in line:
                errors.append({
                    'line': i,
                    'message': 'Найден восклицательный знак',
                    'position': line.find('!') + 1
                })
        
        result_text = f"""{self.tr('=== РЕЗУЛЬТАТЫ АНАЛИЗА ===')}

{self.tr('Общая статистика:')}
- {self.tr('Количество строк:')} {len(lines)}
- {self.tr('Количество слов:')} {word_count}
- {self.tr('Количество символов (без пробелов):')} {char_count}

{self.tr('Найденные "ошибки":')} {len(errors)}
"""
        if errors:
            result_text += f"\n{self.tr('Список ошибок:')}\n"
            for e in errors:
                result_text += f"{self.tr('Строка')} {e['line']}, {self.tr('Позиция')} {e['position']}: {e['message']}\n"
        
        self.result_text.setPlainText(result_text)
        self.error_table_tab.clear_results()
        for e in errors:
            self.error_table_tab.add_result(e['line'], e['position'], e['message'])
        
        self.status_bar.showMessage(self.tr('Анализ завершен'))
    ...
    def show_help(self):
        help_text = self.tr('=== СПРАВКА ПО ТЕКСТОВОМУ РЕДАКТОРУ ===') + '\n\n' + \
                    ...
                    "- " + self.tr('Табличное отображение ошибок') + "\n" + \
    ...
    def show_about(self):
        QMessageBox.about(
            ...
            self.tr("Версия: 2.0 (с дополнительными функциями)") + "\n" +
            self.tr("Разработчики: Тоха, Александр, Виталя") + "\n\n" +
            self.tr("© 2026 Все права защищены нами.")
        )
