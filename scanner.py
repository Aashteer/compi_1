...
class Token:
    ...
    def get_display_type(self, lang='ru'):
        """Возвращает отображаемое название типа лексемы в зависимости от языка"""
        if lang == 'ru':
    ...
    def get_display_value(self, lang='ru'):
        """Возвращает отображаемое значение лексемы с учетом языка"""
    ...
    def to_table_row(self, lang='ru') -> tuple:
        """Преобразует токен в строку для таблицы результатов с учетом языка"""
        if lang == 'ru':
...
class Scanner:
    """
    Лексический анализатор (сканер) для языка Python (подмножество для if-else)
    """
    ...
    def analyze(self, text: str) -> Dict[str, Any]:
        ...
        Возвращает словарь с токенами и ошибками
        """
        self.tokens = []
        self.errors = []
        self.line = 1
    ...
    def _get_operator_chars(self) -> set:
        """Возвращает множество символов, которые могут быть частью операторов"""
        chars = set()
    ...
    def _get_delimiter_chars(self) -> set:
        """Возвращает множество символов-разделителей (кроме пробелов)"""
        return {d for d in self.DELIMITERS if d not in (' ', '\t', '\n')}
    
    def _handle_whitespace(self):
        """Обработка пробельных символов (пробел, табуляция)"""
        start_pos = self.pos
        value = ''
    ...
    def _handle_comment(self):
        """Обработка комментариев (пропускаем до конца строки)"""
    ...
    def get_token_table_data(self, lang='ru') -> List[tuple]:
        """Возвращает данные для отображения в таблице с учетом языка"""
        return [token.to_table_row(lang) for token in self.tokens]
    
    def get_errors_table_data(self, lang='ru') -> List[tuple]:
        """Возвращает данные об ошибках для отображения в таблице с учетом языка"""
